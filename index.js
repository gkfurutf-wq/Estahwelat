...
أرسل المفاتيح الآن:`;
        ...
        this.bot.onText(/\/stop/, (msg) => {
            ...
            let walletsListMessage = '📋 المحافظ المراقبة حالياً:\n\n';
            for (let i = 0; i < this.wallets.length; i++) {
                const wallet = this.wallets[i];
                const isActive = this.subscriptionIds[i] !== null && !this.rpcFailedWallets.has(i + 1);
                const status = isActive ? '🟢 نشط' : '🔴 متوقف';
                walletsListMessage += `${i + 1}. ${wallet.publicKey.toString()}\n   الحالة: ${status}\n\n`;
            }
            
            walletsListMessage += '📝 أرسل عناوين المحافظ التي تريد إيقاف مراقبتها:\n';
            walletsListMessage += '💡 يمكنك إرسال عنوان واحد أو عدة عناوين (كل عنوان في سطر منفصل)';
            
            this.bot.sendMessage(chatId, walletsListMessage);
            
            // Wait for next message with wallet addresses
            this.bot.once('message', (response) => {
                if (response.chat.id === chatId && !response.text.startsWith('/')) {
                    this.stopSpecificWallets(chatId, response.text);
                }
            });
        });
        
        // Stop monitoring command (all wallets)
        this.bot.onText(/\/stop_monitoring/, (msg) => {
            const chatId = msg.chat.id;
            const stoppedCount = this.stopAllMonitoring();
            
            if (stoppedCount > 0) {
                this.bot.sendMessage(chatId, `⏹️ تم إيقاف مراقبة جميع المحافظ بنجاح\n✅ تم إيقاف ${stoppedCount} اشتراك`);
            } else {
                this.bot.sendMessage(chatId, '⏹️ لا توجد محافظ قيد المراقبة للإيقاف');
            }
        });
